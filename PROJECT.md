...
- v0.2 polish (2026-05-25): (1) **run-timeline min-dwell** — `/runs/[id]` now trails the polled status via a `displayIdx` that advances one step at a time with a ~650ms minimum dwell, so a sub-second backend step (`notifying`) gets a visible "active" moment instead of flashing past between two 1.5s polls (`src/components/run-timeline.tsx`); the WDK readable-stream rewrite remains the deferred proper fix. (2) **Stack-pill dedup** — the apex Stack section now renders from a canonical `src/data/stack.ts` (`SELLING_STACK` = tier 1 + tier 2) in the `zeroindex-site` repo, and this repo's `src/lib/stack.ts` is documented as its cross-repo derived mirror (the two repos can't share a file).

### Next (real follow-ups)

_Both shipped 2026-05-25 (see log above) — run-timeline min-dwell + Stack-pill dedup. No open near-term follow-ups; remaining ideas are Deferred (v0.2+) below._
