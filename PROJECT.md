...
| **Admin auth**         | Single `ADMIN_TOKEN` env var; httpOnly cookie stores an HMAC **derived** from the secret (not the secret itself) + `timingSafeEqual` | One admin, no team, no SSO. The cookie holds `HMAC(ADMIN_TOKEN)` so the raw master secret never reaches the browser and a stolen cookie can't be replayed as the credential. Magic-link arrives when there's a second admin to invite. |
...
| **SSRF guard on enrichment**   | `safeFetch` (`src/lib/safe-fetch.ts`) DNS-resolves the user-supplied URL and rejects loopback/private/link-local/ULA/CGNAT/metadata addresses; every redirect hop is re-validated | The enrichment step fetches an attacker-chosen URL server-side. Validating only the initial host is insufficient — a public URL can 30x-redirect to `169.254.169.254`, so redirects are followed manually with per-hop checks. |
| **Rate limiting**              | Fixed-window counters in a `rate_limits` table (`src/lib/rate-limit.ts`): 10/hr per hashed IP, 5/hr per hashed email; `POST /api/intake` returns 429 with `retry-after` | The endpoint is public and each accepted request costs 2 LLM calls + 2 emails. The 24h dedupe hash only collapses identical resubmissions — it is **not** a rate limit and is defeated by varying one character. IP/email are hashed so no raw PII is stored. |
