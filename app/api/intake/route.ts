...
const Body = z.object({
  ...
  problem: z.string().min(10).max(8_000),
  url: z.string().url().max(2_048).optional().or(z.literal('')),
  lookingFor: z.array(z.string().max(80)).max(15).default([]),
  ...
  contactPref: z.string().max(20).optional().default(''),
  referral: z.string().max(60).optional().default(''),
});
